...
test.describe('Single Locale Navigation', () => {
  test('should have navigation links without locale prefixes', async ({ page }) => {
    await page.goto('/')
    ...
    const blogLink = page.locator('nav a[href="/blog"], .navbar a[href="/blog"]').first()
    await expect(blogLink).toBeVisible()
    
    const aboutLink = page.locator('nav a[href="/about"], .navbar a[href="/about"]').first()
  ...
  test('should display content without locale prefixes in URLs', async ({ page }) => {
    await page.goto('/')
  ...
  test('should have working sidebar navigation on docs page', async ({ page }) => {
    await page.goto('/docs')
    
    // Check for sidebar navigation home link without locale prefix
    const homeLink = page.locator('.sidebar a[href="/"], aside a[href="/"]').first()
