...
test.describe('Sidebar Demo - Docusaurus-like Features', () => {
  test.describe('Category-level Sidebar Position', () => {
    test('category with sidebar_position in index.md appears first in top-level navigation', async ({
      ...
      // Get all top-level sidebar items (direct children of the local nav)
      // These are <li> elements that contain either direct links or nested <ul> for categories
      const topLevelItems = localNav.locator('> li')
  ...
  test.describe('Sidebar Position Ordering', () => {
    test('items are ordered by sidebar_position, not alphabetically', async ({
      ...
      // Wait for the local nav to be present
      const localNav = page.locator('[data-testid="sidebar-local-nav"]')
      await expect(localNav).toBeAttached()
    ...
    test('lower sidebar_position appears first', async ({ page }) => {
      ...
      const localNav = page.locator('[data-testid="sidebar-local-nav"]')
      await expect(localNav).toBeAttached()
      const links = localNav.locator('a[href*="/en/docs/sidebar-demo/"]')
  ...
  test.describe('Sidebar Label', () => {
    test('custom sidebar_label overrides default title', async ({ page }) => {
      ...
      const localNav = page.locator('[data-testid="sidebar-local-nav"]')
      await expect(localNav).toBeAttached()
      const fancyLabel = localNav.locator('a:has-text("Fancy Label ✨")')
  ...
  test.describe('Sidebar Class Name', () => {
    test('custom sidebar_class_name is applied to sidebar item', async ({
      ...
      const itemWithBold = localNav.locator('li.font-bold')
      await expect(itemWithBold).toBeAttached()

      const itemWithWarning = localNav.locator('li.text-warning')
    ...
    test('custom class CSS is actually applied (font-bold)', async ({
      ...
      const itemWithClass = localNav.locator('li.font-bold')
    ...
    test('custom class CSS is actually applied (text-warning color)', async ({
      ...
      const itemWithClass = localNav.locator('li.text-warning')
      await expect(itemWithClass).toBeAttached()
    ...
    test('item with custom class contains the correct link', async ({
      ...
      const itemWithClass = localNav.locator('li.font-bold.text-warning')
  ...
  test.describe('Combined Features', () => {
    test('all demo pages are visible in sidebar', async ({ page }) => {
      ...
      await expect(
        localNav.locator('a[href*="/en/docs/sidebar-demo/custom-class"]'),
      ).toBeAttached()
      // custom-label shows as "Fancy Label ✨"
      await expect(
        localNav.locator('a:has-text("Fancy Label ✨")'),
      ).toBeAttached()
    })
